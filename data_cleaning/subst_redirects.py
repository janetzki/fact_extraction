...
class Substitutor:
    ...
    def substitute_dump(self):
        ...
        with io.open(self.dump_path, 'r', encoding='utf8') as fin, io.open(self.dump_path_new, 'w',
                                                                           encoding='utf8') as fout:
            for line in tqdm(fin, total=total_lines):
                substituted_line = sub._substitute_all(line)
                substituted_line = substituted_line.replace('\n', '')
                fout.write(substituted_line)
                fout.write(u'\n')  # prevents that Windows uses '\r\n' as line separator
